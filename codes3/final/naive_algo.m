...
% Output File 
COMMENT = '19thapril'; % for output figure name
...
if RE_TRAIN ~= 1,
    ...
    phrase_collect = mksqlite('SELECT phrase FROM phrase ORDER BY pid'); 
    ...
    if NORMALIZED ==1,
        ...
        if (TEST_NUM == 0)&&(TRAIN_NUM == 0),
            TRAIN_NUM = floor(0.7* value_sort(EMOJI_SELEC));
            TEST_NUM = value_sort(EMOJI_SELEC) - TRAIN_NUM;
        end
    ...
    P_ew_1 = zeros(EMOJI_NUM, WORD_NUM + PHRASE_NUM); % e := emoji, w:=words; prbability of word=1 given emoji
    ...
    P_rank = P_emoji;
    P_rank(idx(1:100)) = mean(P_emoji(idx(1:100)));
    P_table = prod([P_ew_0 P_top100], 2);
...
if TESTING_OR_PREDICT, 
    ...
    for idx = 1:TEST_NUM
        ...
        if ismember(data_test(idx).eid, emoji_table)
            ...
            result(4:PREDICT_NUM+3, idx) = emoji_table(1:20); % first 20 prediction
    ...
    save(sprintf('Result_%s', MAT_FILE), 'result', 'hit_result', 'hit_prior_base', 'rank_result', 'rank_prior_base', 'each_class');
    ...
    while(1)
        ...
        word_match
        display_emojis(emoji_table(1:100), emoji_dic);
    end
end