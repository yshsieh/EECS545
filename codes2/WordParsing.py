...
def wordHash(word):
	...
	if (not endIsRemoved and len(wordHashed)>3):
		wordEnd = wordHashed[-3:]
		if (wordEnd=='ied' or wordEnd=='ies'):
			wordHashed = wordHashed[:-3]
			endIsRemoved = True


	if (not endIsRemoved and len(wordHashed)>2):
		wordEnd = wordHashed[-2:]
		if (wordEnd=='ed' or wordEnd=='es'):
			wordHashed = wordHashed[:-2]
			endIsRemoved = True

	if (not endIsRemoved and len(wordHashed)>1):
		wordEnd = wordHashed[-1:]
		if (wordEnd=='d' or wordEnd=='s' or wordEnd == 'y'):
			wordHashed = wordHashed[:-1]
			endIsRemoved = True
...
def estimateFreq(isWord = True, wordNeedToBeHashed = True):
	...
	WORD_USED_CNT = 5000 # the number of word going to be used
	PHRASE_USED_CNT = 1000

	#limitSuffix = ' limit 200000'
	limitSuffix = ' limit 1000000'
	...
	for (sentence,eid) in zip(sentences,eids):
		...
		if progressNow - progressPre >= 0.1:
			print "progressNow = %f %%" % (progressNow*100)
	...
	phraseKeyWords = ['not','dont','cant','didnt'] # only collect phrase in these phrase
