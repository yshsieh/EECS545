...
def parseController():
	...
	randomSql = 'select name from user where is_parsed = 0 order by random() limit 1'
	for parseCnt in range(MAX_TO_PARSE):
		randomResult = fetchListFromSql(randomSql)

		print randomResult
		# return if there is no avaialble unparesed users
		if(len(randomResult) == 0):
			print '[ERROR]: all the users in the sql have been parsed!'
			return

		name = randomResult[0]
		#print str(name)

		debugShow('start to parse (parseCnt,name) = (%d,%s)' % (parseCnt, name), True, True)
		parse(name, MAX_USER_TO_ADD)
