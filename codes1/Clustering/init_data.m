...
mksqlite('open', '/Users/sammui/Documents/MATLAB/EECS545_Final_Project/mksqlite-1.11-src/instagram.sqlite');
...
data = mksqlite(['SELECT * FROM emoji_text_mapping WHERE eid IN (SELECT eid FROM emoji_text_mapping GROUP BY eid ORDER BY count(eid) DESC LIMIT ' num2str(num_emoji) ') LIMIT 20000']);
...
training_target_value = zeros(1,train_num);
...
save('init_data','training_feature_vector','training_target_vector','training_target_value','testing_feature_vector','testing_target_vector','train_num','dictionary','emojis_dic','-v7.3');
mksqlite('close');
